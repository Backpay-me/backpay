...
day-level **counters only**, and the data union sells k-anonymous aggregates — the live index of
...
while it's still small enough to check by hand.

**Payout rail: Bitcoin Lightning** — money nobody can freeze. The plan is automatic
payout to a Bitcoin Lightning address you control. No wallet yet? [Wallet of
Satoshi](https://www.walletofsatoshi.com/) hands you a Bitcoin Lightning address in
seconds. The *pool* is audited on-chain (inflows + total outflow on a public Bitcoin
address); Bitcoin Lightning handles last-mile delivery.

**One human, one payout.** Automatic, unfreezable payout has to resist Sybil (one person
minting many identities). The gate sits at *payout*, not accrual: **GitHub OAuth** (an
